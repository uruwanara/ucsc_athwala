...
export default function JobList(){
    ...
    const fetchData = async () => {
        ...
        const response = await fetch(`http://localhost:5000/api/auction/mybid`, {
          method: "GET",
          params: {id:stid},
        });
    ...
    function Tablerow(props){
        const auction_id = props.id;
        if(userData.userType == "STUDENT"){
          var viewlink = "/std/jobview?id="+auction_id;
                return(
                    <TableRow key={props.id}>
                        ...
                        <TableCell align="center">{props.baseprice}</TableCell>
        ...
        <div>
            ...
            <TableContainer component={Paper}>
                <Table className={classes.table} aria-label="a dense table">
                    ...
                    <TableBody>
                    {mapset.map((row) => (
                        <Tablerow id={row.auction_id} title={row.title} year={row.year} baseprice = {row.baseprice}/>
                    ))}
                    </TableBody>
                </Table>
            </TableContainer>
